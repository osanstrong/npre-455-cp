...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Considering the one-dimensional, two-region problem, for which the source $S$ is only relevant to the fast group $g=1$ in region $C$, converts this to a set of four distributions (which we refactor using removal cross-sections and diffusion length $L_g^2=\frac{D_g}{\SigRem}$):
\begin{align}
    \pd{\phi_{C1}}{x}{2} - \frac{1}{L_{C1}^2}\phi_{C1} &= -\frac{S_{C1}}{D_{C1}}\label{eq:c1_flux}\\
    \pd{\phi_{C2}}{x}{2} - \frac{1}{L_{C2}^2}\phi_{C2} &= -\Sigma_{sC1\rightarrow2}\phi_{C1}\label{eq:c2_flux}\\
    \pd{\phi_{M1}}{x}{2} - \frac{1}{L_{M1}^2}\phi_{M1} &= 0 \label{eq:m1_flux}\\
    \pd{\phi_{M2}}{x}{2} - \frac{1}{L_{M2}^2}\phi_{M2} &= -\Sigma_{sM1\rightarrow2}\phi_{M1}\label{eq:m2_flux}
\end{align}

As anticipated from earlier discussion, Eqs.~\ref{eq:c1_flux} and \ref{eq:m1_flux} are independent of other distributions, and can be solved first.
...
\begin{equation}
    \begin{gathered}
        \phi_{C1}=\phi_h + \phi_p\\
        \pd{\phi_h}{x}{2} - \frac{1}{L_{C2}^2}\phi = -\frac{S}{D_{C2}}
    \end{gathered}
\end{equation}
