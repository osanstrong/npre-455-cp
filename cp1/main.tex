...
\begin{itemize}
    \item Flux described by both regions' $\phi_i\rt$ should agree at their interface: $\phi_a\rt=\phi_b\rt,{\hat{r}\in S_{ab}}$
\end{itemize}

\FloatBarrier

\defplot{prob2_setup}{The (symmetric) geometry of Problem~2: A central source, then one slab of Material~A, then a slab of Material~B before vacuum.}
...
Thus, we return to the 1D system in \figref{prob2_setup}.
\subsection{Analytical Solution}
To analytically solve for this distribution, one may first split the problem into two equations, one for each materially uniform region.
...
Eq.~\ref{eq:reg1-source} is not homogeneous (having the term $\frac{S}{D_1}$ independent of flux $\phi_1$), but can be split into a homogenous component solution $\phi_h$ and a particular solution $\phi_p$
\begin{equation}
    \label{eq:homo-part}
    \phi_1 = \phi_h + \phi_p
\end{equation}
...
\end{align}
This solution, for NUMBERS can be seen in \figref{P2_flux}, compared to varying numerical solutions found in the next section.
...
\end{split}
\end{equation}
Taking a linear regression of $\log(\mathrm{error})$ and $\log(h)$ found using solutions with $N\in\left\{25, 50, 100, 200, 400, 800, 1600\right\}$ cells, an order of convergence was found of $\bigO(h^{1.00912})$, slightly better than first order.
The errors found for different values of $h$ may be seen on a log-log plot in \figref{P2_error}.\\

To estimate error, a relative L$^2$ norm was computed for each flux solution $\phi$ and the exact analytical solution $\phi^*$:
\begin{equation}
    \label{eq:err-def}
...
Suppose that for some flux distribution $\phi$, the distribution time change $\frac{1}{v}\frac{\partial \phi}{\partial t}$ is proportional to the current distribution of flux, such that on average every neutron released goes on to cause $k$ subsequent neutron releases through fission, a multiplication rate $k$.
...
\begin{equation}
    ...
    B=\frac{\nu\Sigma_f - \Sigma_a}{D}
\end{equation}
This ``material'' buckling can then be compared to the ``geometric'' buckling found through the geometry of the problem to determine what materials or what geometry would be required to produce a critical system (if such a feat is posible).

For a non-critical system of determined geometry and material, if the unknown multiplication rate $k$ is folded into this definition, one can define a hypothetically critical material buckling $B_c$:
\begin{equation}
...
Because $B_c$ corresponds to an individual $k$ value (Eq.~\ref{eq:crit-buckling}), this condition typically reveals an infinite series of $B_c$.
...
For instance, if we wanted a geometry which was perfectly critical, then we could impose that $B_c=B$, since $k=1$
